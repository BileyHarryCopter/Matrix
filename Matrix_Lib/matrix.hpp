...
public:
    ...
    Matrix(Matrix &&rhs) noexcept : n_{rhs.n_}, m_{rhs.m_}, rows_{std::move(rhs.rows_)}
    {
        for (auto i = 0; i < m_; ++i)
            rows_[i] = std::exchange(rhs.rows_[i], nullptr);
        rhs.rows_ = nullptr;
    }
...
//  some useful methods
    bool operator== (const Matrix& rhs) const
    {
        ...
        else
        {
            for (auto i = 0; i < m_; ++i)
            {
                for (auto j = 0; j < n_; ++j)
                {
                    if (cmp::are_equal(rows_[i][j], rhs.rows_[i][j], cmp::epsilon) == false)
                        return false;
                }
            }
        }
    ...
    Matrix& operator/= (T value)
    {
        //  Check if value is zero for all types
        ...
        if (std::is_integral<T>::value == false)
        {
            ...
            for (auto i = 0; i < n_ - 1; ++i)
            {
                ...
                if (tmp.rows_[i][i] == 0.0) 
                {
                    ...
                    auto pivot_row = tmp.rows_[pivot_i];
                    tmp.rows_[pivot_i] = tmp.rows_[i];
                    tmp.rows_[i] = pivot_row;
                ...
                for (auto j = i + 1; j < n_; ++j)
                {
                    ...
                    for (auto k = i; k < n_; ++k)
                        tmp.rows_[j][k] -= koef * tmp.rows_[i][k];
                }
            }
